...
def convert_enhanced_to_simple(enhanced_json_path: str, output_path: str):
    ...
    for block in enhanced_blocks:
        ...
        simple_para = {
            'index': block['index'],
            'list_number': block['numbering_pattern'] or '',
            'inferred_number': block['inferred_number'],
            'text': block['cleaned_content'],  # Use cleaned content
            'combined': block['text'],  # Keep original combined text
            'level': block['level'],
