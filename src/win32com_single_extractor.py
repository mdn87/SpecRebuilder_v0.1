#!/usr/bin/env python3
"""
Win32COM Single Document Extractor

This script uses win32com.client to extract numbered paragraphs directly from Word
documents and analyze their structure, without requiring a text file for comparison.
"""
...
class Win32COMSingleExtractor:
    """Extracts and analyzes numbered paragraphs from Word documents using win32com"""
    ...
    def analyze_document_structure(self, paragraphs: List[NumberedParagraph]) -> DocumentAnalysis:
        ...
        unnumbered_count = len(paragraphs) - numbered_count
    ...
    def generate_analysis_report(self, docx_path: str) -> Dict[str, Any]:
        ...
        report = {
            ...
            'structure_analysis': {
                ...
                'numbering_percentage': (analysis.numbered_paragraphs / analysis.total_paragraphs * 100) if analysis.total_paragraphs > 0 else 0
            },
    ...
    def print_analysis_summary(self, report: Dict[str, Any]):
        ...
        if patterns:
            ...
            print("  No numbering patterns found")
        ...
        for i, para in enumerate(report['sample_paragraphs']):
            ...
            print(f"{i+1:2d}. [{numbering:8}]{level_info} '{para['text']}'")
...
def main():
    """Main function"""
    if len(sys.argv) < 2:
        print("Usage: python win32com_single_extractor.py <docx_file> [output_dir]")
        sys.exit(1)
    ...
    try:
        extractor = Win32COMSingleExtractor()
        
        print(f"Analyzing Word document: {docx_path}")
        
        # Generate analysis report
        report = extractor.generate_analysis_report(docx_path)
        
        # Print summary
        extractor.print_analysis_summary(report)
        
        # Save report
        base_name = Path(docx_path).stem
        output_path = os.path.join(output_dir, f"{base_name}_win32com_analysis.json")
        ...
        print(f"Error in Win32COM analysis: {e}")
        import traceback
        traceback.print_exc()
        sys.exit(1)
